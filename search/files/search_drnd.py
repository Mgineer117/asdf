...
def train():
    ...
    if "drnd_lr" in config:
        args.drnd_lr = config.drnd_lr
    if "int_reward_scaler" in config:
        args.int_reward_scaler = config.int_reward_scaler
    if "ext_reward_scaler" in config:
        args.ext_reward_scaler = config.ext_reward_scaler
    ...
    print(f"-------------------------------------------------------")
    print(f"      DRND {search_args.env} Sweep Trial ID: {unique_id}")
    print(f"      Seed: {seed}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WandB Sweep Launcher for DRND")
    ...
    if search_args.project is None:
        search_args.project = f"DRND-{search_args.env.upper()}-SWEEP"
    ...
    if search_args.sweep_id is None:
        sweep_config = {
            ...
            "parameters": {
                ...
                # Additional parameters worth searching:
                # "ext_reward_scaler": {
                #     "min": 0.5,
                #     "max": 5.0,
                #     "distribution": "log_uniform_values"
        ...
        print(f"python search_drnd.py --sweep_id {sweep_id} --env {search_args.env}")
        print(f"=======================================================\n")
