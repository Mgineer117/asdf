...
def train():
    ...
    print(f"-------------------------------------------------------")
    print(f"      PSNE {search_args.env} Sweep Trial ID: {unique_id}")
    print(f"      Seed: {seed}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WandB Sweep Launcher for PSNE")
    ...
    if search_args.project is None:
        search_args.project = f"PSNE-{search_args.env.upper()}-SWEEP"
    ...
    if search_args.sweep_id is None:
        ...
        print(f"python search_psne.py --sweep_id {sweep_id} --env {search_args.env}")
        print(f"=======================================================\n")
