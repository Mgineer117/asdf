    ...
    def _create_hindsight_batch(
        ...
        for g_idx, g_prime in enumerate(G):
            ...
            if self.env_name in ["fetchreach", "fetchpush"]:
                ...
                hs_batch["rewards"][:] = 0.0
                hs_batch["rewards"][reached_mask] = 1.0
                valid_mask = np.ones(n_orig, dtype=bool)
                ...
                if "terminations" in hs_batch:
                    ...
                    for ep_s, ep_e in zip(ep_starts, ep_ends_clean):
                        ep_reached = reached_mask[ep_s : ep_e + 1]
                        if np.any(ep_reached):
                            first_reach_idx = ep_s + np.argmax(ep_reached)
                            new_terms[first_reach_idx] = True
                            if first_reach_idx < ep_e:
                                valid_mask[first_reach_idx + 1 : ep_e + 1] = False

                    hs_batch["terminations"] = new_terms.astype(np.float32)

            valid_idx = np.where(valid_mask)[0]
            for k in batch.keys():
                augmented_batches[k].append(hs_batch[k][valid_idx])
            goal_ids_list.append(np.full(len(valid_idx), g_idx + 1, dtype=np.int64))
